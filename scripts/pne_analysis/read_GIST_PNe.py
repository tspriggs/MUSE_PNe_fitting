...
# extract PN vel
PN_vel = np.array([gandalf_results[2].data[i]["V"][OIII_ind] for i in np.arange(len(gandalf_results[2].data))])


v_star     = np.array(ppxf_results[1].data["V"])
v_star_arr = np.ones_like(PN_vel)
for n in np.arange(len(v_star)):
    v_star_arr[n] = v_star[n]

sigma_star = np.array(ppxf_results[1].data["SIGMA"])
sigma_star_arr = np.ones_like(PN_vel)
for n in np.arange(len(sigma_star)):
    sigma_star_arr[n] = sigma_star[n]


v_star_arr[v_star_arr==1.] = np.nan
sigma_star_arr[sigma_star_arr==1.] = np.nan

gist_df = PNe_df.loc[PNe_df["ID"]=="PN"]
f_ind = gist_df.index.values # index for filtered PN
f_ind = np.delete(f_ind, np.squeeze(np.where(f_ind == np.where(np.isnan(PN_vel)))), 0)

vel_ratio = (PN_vel - v_star_arr) / sigma_star_arr
interlopers = np.where((vel_ratio <-3) | (vel_ratio > 3))[0]
to_plot = np.where((vel_ratio >-3) & (vel_ratio < 3))[0]

binwidth=0.5
plt.figure(figsize=(10,8))
if len(interlopers) >= 1:
    plt.hist(vel_ratio[to_plot], bins=10, edgecolor="k", alpha=0.8, linewidth=1) # plot without interlopers
    plt.xlim(-3.25,3.25)
    # plt.hist(vel_ratio[to_plot], bins=np.arange(min(vel_ratio[to_plot]), max(vel_ratio[to_plot]) + binwidth, binwidth), edgecolor="k", alpha=0.8, linewidth=1) # plot without interlopers
    # plt.hist(vel_ratio[interlopers], bins=10, edgecolor="k", alpha=0.8, linewidth=1) # plot of just interlopers
else:
    plt.hist(vel_ratio, bins=10, edgecolor="k", alpha=0.8, linewidth=1) # plot without interlopers
    plt.xlim(-3.25,3.25)
...
print(f"Objects {interlopers} identified as interlopers")
# save the GIST derived PN velocities
np.save(DIR_dict["EXPORT_DIR"]+"_PNe_vel", PN_vel)
...
# Sigma limiting
OIII_sigma = np.array([gandalf_results[2].data[i]["Sigma"][OIII_ind] for i in np.arange(len(gandalf_results[2].data))])
OIII_sigma[np.isnan(OIII_sigma)] = 0.0
# sigma_filter = np.where((OIII_sigma<80.0)&(OIII_sigma>10.0))[0]

OIII_vel = np.array([gandalf_results[2].data[i]["V"][OIII_ind] for i in np.arange(len(gandalf_results[2].data))])
OIII_vel[np.isnan(OIII_vel)] = 0.0
...
# OIII_flux   = np.array([gandalf_results[2].data[i]["Flux"][OIII_ind] for i in np.arange(len(gandalf_results[2].data))])* 1e-20
# Ha_flux     = np.array([gandalf_results[2].data[i]["Flux"][Ha_ind] for i in np.arange(len(gandalf_results[2].data))])* 1e-20
# NII_flux    = np.array([gandalf_results[2].data[i]["Flux"][NII_ind] for i in np.arange(len(gandalf_results[2].data))])* 1e-20
# SII_I_flux  = np.array([gandalf_results[2].data[i]["Flux"][SII_I_ind] for i in np.arange(len(gandalf_results[2].data))])* 1e-20
# SII_II_flux = np.array([gandalf_results[2].data[i]["Flux"][SII_II_ind] for i in np.arange(len(gandalf_results[2].data))])* 1e-20

# fluxes corrected for profile shape.
# Hb_flux     = np.array([gandalf_results[2].data[i]["Ampl"][11]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][12]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
OIII_flux   = np.array([gandalf_results[2].data[i]["Ampl"][OIII_ind]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][OIII_ind]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
# OIII_flux   = np.array([gandalf_results[2].data[i]["Ampl"][OIII_ind]*np.sqrt(2*np.pi)*(((40*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
Ha_flux     = np.array([gandalf_results[2].data[i]["Ampl"][Ha_ind]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][OIII_ind]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
NII_flux    = np.array([gandalf_results[2].data[i]["Ampl"][NII_ind]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][OIII_ind]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
SII_I_flux  = np.array([gandalf_results[2].data[i]["Ampl"][SII_I_ind]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][OIII_ind]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
SII_II_flux = np.array([gandalf_results[2].data[i]["Ampl"][SII_II_ind]*np.sqrt(2*np.pi)*(((gandalf_results[2].data[i]["Sigma"][OIII_ind]*1e3)  / 299792458.0)*5006.77) for i in np.arange(len(gandalf_results[2].data))]) * 1e-20
...
gand_df.loc[indx, "[OIII] sigma"]    = OIII_sigma
gand_df.loc[indx, "[OIII] amp"]      = OIII_ampl
gand_df.loc[indx, "[OIII] flux"]     = OIII_flux
gand_df.loc[indx, "[OIII] AON"]      = OIII_AON
gand_df.loc[indx, "Ha amp"]          = Ha_ampl
gand_df.loc[indx, "Ha flux"]         = Ha_flux
gand_df.loc[indx, "Ha AON"]          = Ha_AON
gand_df.loc[indx, "[NII] amp"]       = NII_ampl
gand_df.loc[indx, "[NII] flux"]      = NII_flux
gand_df.loc[indx, "[NII] AON"]       = NII_AON
gand_df.loc[indx, "[SII]I amp"]      = SII_I_ampl
gand_df.loc[indx, "[SII]I flux"]     = SII_I_flux
gand_df.loc[indx, "[SII]I AON"]      = SII_I_AON
gand_df.loc[indx, "[SII]II amp"]     = SII_II_ampl
gand_df.loc[indx, "[SII]II flux"]    = SII_II_flux
gand_df.loc[indx, "[SII]II AON"]     = SII_II_AON

gand_df.replace(0, np.nan, inplace=True)
...
dM = gal_df.loc[(galaxy_name, "center"), "PNLF dM"]
# dM = gal_df.loc[(galaxy_name, "center"), "Bl dM"]
...
HaNII_or_cond = ((gand_df["Ha AON"] >= 3.) | (gand_df["[NII] AON"]>=3.)) & (gand_df["ID"]!="-") & (gand_df["[OIII] sigma"]<80.0)&(gand_df["[OIII] sigma"]>15.0)

# Ha and NII both over 3 times the signal to noise, and ID is PN
HaNII_and_cond = ((gand_df["Ha AON"] >= 3.) & (gand_df["[NII] AON"]>=3.)) & (gand_df["ID"]!="-") & (gand_df["[OIII] sigma"]<80.0)&(gand_df["[OIII] sigma"]>15.0)
...
ratio = gand_df["[OIII] flux"] / (gand_df["Ha flux"]+ 1.34*gand_df["[NII] flux"])

# Make a copy of NII flux array
F_NII_corr = np.copy(gand_df["[NII] flux"])

# find where NII AON is less than 3
NII_AON_lt_3 = gand_df["[NII] AON"] < 3.

# Where NII AON is less than 3: do 3 * F_NII / AON_NII
F_NII_corr[NII_AON_lt_3] = 3*(gand_df.loc[NII_AON_lt_3,"[NII] flux"] / gand_df.loc[NII_AON_lt_3, "[NII] AON"])

# Now calculate the ratio correction: F_[OIII] / F_Ha + (1.34 * corrected F_NII)
ratio_corr = np.copy(gand_df["[OIII] flux"]) / (np.copy(gand_df["Ha flux"]) + 1.34 * F_NII_corr)
...
HaoNII = gand_df["Ha flux"] / gand_df["[NII] flux"]
HaoSII = gand_df["Ha flux"] / (gand_df["[SII]I flux"] + gand_df["[SII]II flux"])

SII_I_lt_3 = gand_df["[SII]I AON"] < 2.5
SII_II_lt_3 = gand_df["[SII]II AON"] < 2.5

SII_I_F_corr = np.copy(gand_df["[SII]I flux"])
SII_II_F_corr = np.copy(gand_df["[SII]II flux"])

SII_I_F_corr[SII_I_lt_3] = 3 * (gand_df.loc[SII_I_lt_3, "[SII]I flux"] / gand_df.loc[SII_I_lt_3, "[SII]I AON"])
SII_II_F_corr[SII_II_lt_3] = 3 * (gand_df.loc[SII_II_lt_3, "[SII]II flux"] / gand_df.loc[SII_II_lt_3, "[SII]II AON"])

HaoSII_corr = gand_df["Ha flux"] / (SII_I_F_corr+SII_II_F_corr)


################################ Filter Ha/NII vs Ha/SII data ################################
# F_Ha / F_NII where Ha and NII are detected above AON of 3, and ID == PN
Ha_NII = gand_df.loc[HaNII_and_cond,"Ha flux"].values / gand_df.loc[HaNII_and_cond, "[NII] flux"].values

# F_Ha / (F_SII I + F_SII II), where Ha and NII are detected above AON of 3, and ID == PN
Ha_SII = gand_df.loc[HaNII_and_cond, "Ha flux"].values / (gand_df.loc[HaNII_and_cond, "[SII]I flux"].values + gand_df.loc[HaNII_and_cond, "[SII]II flux"].values)

# F_Ha / F_NII where Ha and NII are detected above AON of 3, along with combined SII detected above 2.5, and ID == PN
Ha_NII_1 = gand_df.loc[HaNII_SII_and_cond, "Ha flux"].values / gand_df.loc[HaNII_SII_and_cond, "[NII] flux"].values

# F_Ha / (F_SII I + F_SII II), where Ha and NII are detected above AON of 3, along with combined SII detected above 2.5, and ID == PN
Ha_SII_1 = gand_df.loc[HaNII_SII_and_cond, "Ha flux"].values / (gand_df.loc[HaNII_SII_and_cond, "[SII]I flux"].values + gand_df.loc[HaNII_SII_and_cond, "[SII]II flux"].values)
...
# mag_range = np.arange(26, 30, 0.001)
...
sig_em = np.array([gandalf_results[2].data[i]["Sigma"][OIII_ind] for i in np.arange(len(m_5007))])

plt.scatter(gand_df.loc[HaNII_or_cond, "m 5007"].values, HaNII_ratio_cond_or, c=gand_df.loc[HaNII_or_cond, "Ha AON"], vmin=3, vmax=np.max(gand_df.loc[HaNII_or_cond, "Ha AON"]), s=p_s)


#### NEED TO ADD IN CONDITION of Ha or NII >3
for i in np.where(HaNII_or_cond)[0]:
    if gand_df["[NII] AON"].iloc[i] < 3:
...
for n, i in enumerate(gand_df.loc[HaNII_or_cond, "[OIII] flux"].index.values):
...
plt.scatter(gand_df.loc[HaNII_SII_or_cond, "m 5007"].values, HaNII_ratio_SII, facecolor="None", edgecolor="k", lw=1.2, s=p_s+150, label=r"$A_{[SII]}/rN > 2.5$")
...
plt.scatter(Ha_SII, Ha_NII, c=gand_df.loc[HaNII_and_cond, "[NII] AON"], s=p_s, vmin=3, vmax=6, edgecolors="k", lw=0.8)
...
for i in np.where(HaNII_and_cond)[0]:
    if (gand_df["[SII]I AON"].iloc[i] <3) | (gand_df["[SII]II AON"].iloc[i]<3):
...
for n, i in enumerate(gand_df.loc[HaNII_and_cond, "Ha flux"].index.values):
    if ann == True:
...
m = gand_df.loc[HaNII_and_cond, "m 5007"].values
...
def emission_plot_maker(obj_n, obj_t, shift=0, save_fig=False):
    ...
    f_ax1.set_ylim(np.min(AllSpectra[1].data[obj_n][0])-5e2, np.max(AllSpectra[1].data[obj_n][0])+5e2)
    ...
    residuals = AllSpectra[1].data[obj_n][0] - gandalf_best[1].data[obj_n][0]
    f_ax2.plot(wave, gandalf_emission[1].data[obj_n][0], lw=1.5, c="blue", label="emission") # emission
    f_ax2.scatter(wave, residuals, c="k", s=1, label="residuals")  # residuals
    f_ax2.axhline(np.std(residuals), c="k", ls="--", alpha=0.7, label="residual noise") # residual noise line
    f_ax2.set_ylim(-1000, np.max(gandalf_emission[1].data[obj_n][0])*1.25)
    f_ax2.set_xlim(min(wave)-30, max(wave)+30)
    ...
    sub_OIII = np.min(AllSpectra[1].data[obj_n][0])*1.15
    ...
    f_ax3.set_ylim(-900, np.max(AllSpectra[1].data[obj_n][0][ind_OIII[0]:ind_OIII[1]])-sub_OIII+5e2)
    ...
    sub_Ha = np.min(AllSpectra[1].data[obj_n][0])*1.3
    ...
    f_ax4.set_ylim(-900, np.max(AllSpectra[1].data[obj_n][0][ind_Ha[0]:ind_Ha[1]])-sub_Ha+5e2)
...
# PNe Plot
# FCC167
# emission_plot_maker(p, obj_t="PNe",)
# FCC219
...
if len(SII_II_AON[SNR]) != 0:
    ...
    emission_plot_maker(n, obj_t="SNR",)
