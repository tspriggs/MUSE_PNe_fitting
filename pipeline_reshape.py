...
huduu = fits.open(directory + directory + galaxy + '_AllSpectra.fits')
...
hdu1, hdu2      = fits.open(directory + directory+galaxy+'_gandalf-residuals_SPAXEL.fits'), fits.open(directory + directory+galaxy+'_gandalf-emission_SPAXEL.fits')
data1, data2    = hdu1[1].data, (hdu2[1].data)
resid, emission = data1['RESIDUALS'], data2['EMISSION'].T

hdu  = fits.open(directory + directory+galaxy+'_gandalf_SPAXEL.fits')
data = hdu[2].data
AoN  = data['AoN'][:,11]
...
save_list(resid_list, hdrr["LOGLAM"], directory+"reshaped_cubes/"+galaxy+"_residuals_list.fits")