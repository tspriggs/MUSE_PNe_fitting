...
def ppxf_L_tot(int_spec, header, redshift, vel, dist_mod, dM_err=[0.1,0.1]):
    ...
    result_dict = {"Lbol":lum_bol_g, "Lbol_err_up":lum_bol_g_u, "Lbol_err_lo":lum_bol_g_l, 
                    "mag_g":mag_g, "mag_r":mag_r, "mag_v":mag_v, "sigma":fit_sigma[1]}
...
def transmission(lamb, spectra, band):
    ...
    # load up the band filter .dat file
    if band in ["r", "g"]:
    ...
    total_flux = np.sum(spectra)*dl
    total_flux_band = np.sum(spectra_band)*dl
